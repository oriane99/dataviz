...
with col1 :
    ...
    #2e graphe
    fig = xp.bar(salaire_maximal, x='Job Title', y='Max_Salary', color='Location', barmode='group',title='Salaire maximal par job en fonction de la localisation')
...
with col2 :
    ...
    # 2e graphe
    fig = xp.bar(df, x='Company Name', color='Size', color_discrete_map=colors, title='Distribution de la taille des entreprises ')
    fig.update_xaxes(title_text=None)
    st.plotly_chart(fig)

    #3e graphe
    fig_max_salary = xp.histogram(
        df,
        x='Max_Salary',
...
#4e graphe
fig = xp.scatter(taux_moyen, x='Job Title', y='Rating', color='Rating', hover_data=['Job Title'],
...
#5e graphe
fig = xp.histogram(easy, x='Company Name', color='Easy Apply', histnorm='percent',title='Les entreprises qui acceptent facilement les dépots de candidatures')
fig.update_xaxes(title_text=None)
st.plotly_chart(fig, use_container_width=True)
...
fig = xp.bar(top_10_entreprise, x='Company Name', y='Count', title='Top 10 des entreprises qui embauchent les data Analaystes')
fig.update_xaxes(title_text=None)
fig.update_yaxes(title_text=None)
st.plotly_chart(fig)
